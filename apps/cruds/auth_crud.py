...
from apps.cruds import system_crud
from apps.models import user_model, role_model, menu_model, dept_model, auth_user_to_role_model
from apps.models.help_issue import HelpIssue
from apps.routers.auth.params import UserParams
...
class UserDal(DalBase):
    ...
    def __init__(self, db: AsyncSession):
        ...
        self.schema = user_schema.UserSimpleOut

    async def recursion_get_dept_ids(
            self,
            user: user_model.UserModel,
            depts: list[user_model.DeptModel] = None,
    ...
    async def update_login_info(self, user: user_model.UserModel, last_ip: str) -> None:
        """
        更新当前登录信息
        :param user: 用户对象
    ...
    async def create_data(
        ...
        password = data.telephone[
                   5:12] if settings.settings.auth.DEFAULT_PASSWORD == "0" else settings.settings.auth.DEFAULT_PASSWORD
        data.password = self.model.get_password_hash(password)
        data.avatar = data.avatar if data.avatar else settings.settings.auth.DEFAULT_AVATAR
    ...
    async def reset_current_password(self, user: user_model.UserModel, data: user_schema.ResetPwd) -> None:
        """
        重置密码
        :param user:
        :param data:
        ...
        if data.password != data.password_two:
            raise CustomException(message="两次密码不一致")
        result = test_password(data.password)
        if isinstance(result, str):
            raise CustomException(message=result)
        user.password = self.model.get_password_hash(data.password)
        user.is_reset_password = True
        await self.flush(user)

    async def update_current_info(self, user: user_model.UserModel, data: user_schema.UserUpdateBaseInfo) -> Any:
        """
        更新当前用户基本信息
        :param user:
        :param data:
    ...
    async def get_export_headers_options(self, include: list[str] = None) -> dict[str, list]:
        ...
        if include is None or 'gender' in include:
            gender_objs = await system_crud.DictTypeDal(self.db).get_dicts_details(["sys_vadmin_gender"])
    ...
    async def get_import_headers_options(self) -> None:
        ...
        # 性别选择项
        gender_options = await system_crud.DictTypeDal(self.db).get_dicts_details(["sys_vadmin_gender"])
    ...
    async def init_password(self, ids: list[int]) -> list:
        ...
        for user in users:
            ...
            password = user.telephone[5:12] \
                if settings.settings.auth.DEFAULT_PASSWORD == "0" \
                else settings.settings.auth.DEFAULT_PASSWORD
    ...
    async def update_current_avatar(self, user: user_model.UserModel, file: UploadFile) -> str:
        """
        更新当前用户头像
        :param user:
        :param file:
        :return:
        """
        result = await AliyunOSS(BucketConf(**settings.settings.oss.ALIYUN_OSS)).upload_image("avatar", file)
        user.avatar = result
        await self.flush(user)
        return result

    async def update_wx_server_openid(self, code: str, user: user_model.UserModel, redis: Redis) -> bool:
        """
        更新用户服务端微信平台openid
        :param code:
        :param user:
...
class RoleDal(DalBase):
    ...
    def __init__(self, db: AsyncSession):
        super(RoleDal, self).__init__(db, role_model.RoleModel, role_schema.RoleSimpleOut)
    ...
    async def get_select_datas(self) -> list:
        ...
        return [role_schema.RoleOptionsOut.model_validate(i).model_dump() for i in queryset.all()]
    ...
    async def delete_datas(self, ids: list[int], v_soft: bool = False, **kwargs) -> None:
        ...
        :param kwargs: 其他更新字段
        """
        user_count = await UserDal(self.db).get_count(v_join=[["roles"]], v_where=[role_model.RoleModel.id.in_(ids)])
        if user_count > 0:
...
class MenuDal(DalBase):
    ...
    def __init__(self, db: AsyncSession):
        super(MenuDal, self).__init__(db, menu_model.MenuModel, menu_schema.MenuSimpleOut)
    ...
    async def get_routers(self, user: user_model.UserModel) -> list:
        """
        获取路由表
        ...
        if any([i.is_admin for i in user.roles]):
            ...
            options = [joinedload(user_model.UserModel.roles).subqueryload(role_model.RoleModel.menus)]
    ...
    def generate_router_tree(self, menus: list[menu_model.MenuModel], nodes: filter, name: str = "") -> list:
        """
        生成路由树
        ...
        data = []
        for root in nodes:
            router = menu_schema.RouterOut.model_validate(root)
            router.name = name + "".join(name.capitalize() for name in router.path.split("/"))
            router.meta = menu_schema.Meta(
                title=root.title,
                icon=root.icon,
                hidden=root.hidden,
    ...
    def generate_tree_list(self, menus: list[menu_model.MenuModel], nodes: filter) -> list:
        """
        生成菜单树列表
        ...
        data = []
        for root in nodes:
            router = menu_schema.MenuTreeListOut.model_validate(root)
    ...
    def generate_tree_options(self, menus: list[menu_model.MenuModel], nodes: filter) -> list:
        """
        生成菜单树选择项
...
class DeptDal(DalBase):
    ...
    def __init__(self, db: AsyncSession):
        ...
        self.db = db
        self.model = dept_model.DeptModel
        self.schema = dept_schema.DeptSimpleOut
    ...
    def generate_tree_list(self, depts: list[dept_model.DeptModel], nodes: filter) -> list:
        """
        生成部门树列表
        :param depts: 总部门列表
        ...
        data = []
        for root in nodes:
            router = dept_schema.DeptTreeListOut.model_validate(root)
            sons = filter(lambda i: i.parent_id == root.id, depts)
            router.children = self.generate_tree_list(depts, sons)
            data.append(router.model_dump())
        return data

    def generate_tree_options(self, depts: list[dept_model.DeptModel], nodes: filter) -> list:
        """
        生成部门树选择项
        :param depts: 总部门列表
        :param nodes: 每层节点部门列表
...
class TestDal(DalBase):
    ...
    async def test_session_cache1(self):
        ...
        """
        # 第一次查询，并加载用户的所有关联部门项
        sql1 = select(user_model.UserModel).where(user_model.UserModel.id == 1).options(
            joinedload(user_model.UserModel.depts)
        )
        ...
        sql2 = select(user_model.UserModel).where(user_model.UserModel.id == 1)
    ...
    async def test_session_cache3(self):
        ...
        sql1 = select(user_model.UserModel).options(joinedload(user_model.UserModel.depts))
        queryset1 = await self.db.scalars(sql1)
        ...
        # 第二次即使没有加载用户关联的部门，同样可以访问，因为这里会默认从会话缓存中获取
        sql2 = select(user_model.UserModel)
        queryset2 = await self.db.scalars(sql2)
    ...
    async def test_session_cache5(self):
        ...
        """
        # 第一次查询，并加载用户的所有关联部门项
        sql = select(user_model.UserModel).where(user_model.UserModel.id == 1).options(
            joinedload(user_model.UserModel.depts)
        )
        queryset = await self.db.scalars(sql)
    ...
    async def test_join_form(self):
        ...
        sql = select(dept_model.DeptModel).where(dept_model.DeptModel.is_delete == false())
        sql = sql.join_from(user_model.UserModel, user_model.UserModel.depts).where(
        ...
        # FROM
        # 	vadmin_auth_user
        # 	JOIN auth_user_depts ON vadmin_auth_user.id = auth_user_depts.user_id
        # 	JOIN vadmin_auth_dept ON vadmin_auth_dept.id = auth_user_depts.dept_id
        # WHERE
    ...
    async def test_left_join(self):
        ...
        """
        # 封装查询语句
        dept_alias = aliased(dept_model.DeptModel)
        v_options = [contains_eager(self.model.depts, alias=dept_alias)]
        v_outer_join = [
            [auth_user_to_role_model.auth_user_depts_model,
             self.model.id == auth_user_to_role_model.auth_user_depts_model.c.user_id],
            [dept_alias,
             and_(dept_alias.id == auth_user_to_role_model.auth_user_depts_model.c.dept_id, dept_alias.owner == "张伟")]
        ]
        datas: list[user_model.UserModel] = await self.get_datas(
            limit=0,
        ...
        # 原查询语句：
        # DeptAlias = aliased(models.DeptModel)
        # sql = select(self.model).where(self.model.is_delete == false())
    ...
    async def relationship_where_operations_any(self):
        ...
        """
        print("==============================any 方法使用案例1=========================================")
        # 用户表（models.UserModel）与 部门表（DeptModel）为多对多关系
        # 查找出只有满足关联了部门名称为 "人事一部" 的所有用户，没有关联的则不会查询出来
        sql1 = select(user_model.UserModel).where(
            user_model.UserModel.depts.any(dept_model.DeptModel.name == "人事一部")
        )
        queryset1 = await self.db.scalars(sql1)
        ...
        sql2 = select(user_model.UserModel).where(
            ~user_model.UserModel.depts.any(dept_model.DeptModel.name == "人事一部")
        )
        queryset2 = await self.db.scalars(sql2)
        ...
        print("==============================any 方法使用案例3=========================================")
        # 查询出没有关联部门的所有用户
        sql3 = select(user_model.UserModel).where(~user_model.UserModel.depts.any())
    ...
    async def relationship_where_operations_has(self):
        ...
        """
        print("==============================has 方法使用案例1=========================================")
        # 用户（models.UserModel）与 帮助问题（models.VadminIssue）为多对一关系
        # 查找出只有满足关联了用户名称为 "ranyong" 的所有帮助问题，没有关联的则不会查询出来
        sql1 = select(HelpIssue).where(
            HelpIssue.create_user.has(user_model.UserModel.name == "ranyong")
        )
