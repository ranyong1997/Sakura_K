...
async def create_dict_types(data: schemas.DictType, auth: Auth = Depends(AllUserAuth())):
    return RestfulResponse.success(await crud.DictTypeDal(auth.db).create_data(data=data))
...
async def get_dicts_options(auth: Auth = Depends(AllUserAuth())):
    return RestfulResponse.success(await crud.DictTypeDal(auth.db).get_select_datas())
...
async def put_dict_types(data_id: int, data: schemas.DictType, auth: Auth = Depends(AllUserAuth())):
    return RestfulResponse.success(await crud.DictTypeDal(auth.db).put_data(data_id, data))
...
async def get_dict_type(data_id: int, auth: Auth = Depends(AllUserAuth())):
    ...
    return RestfulResponse.success(await crud.DictTypeDal(auth.db).get_data(data_id, v_schema=schema))
...
async def create_dict_details(data: schemas.DictDetails, auth: Auth = Depends(AllUserAuth())):
    return RestfulResponse.success(await crud.DictDetailsDal(auth.db).create_data(data=data))
...
async def put_dict_details(data_id: int, data: schemas.DictDetails, auth: Auth = Depends(AllUserAuth())):
    return RestfulResponse.success(await crud.DictDetailsDal(auth.db).put_data(data_id, data))
...
async def get_dict_detail(data_id: int, auth: Auth = Depends(AllUserAuth())):
    ...
    return RestfulResponse.success(await crud.DictDetailsDal(auth.db).get_data(data_id, v_schema=schema))
...
async def upload_file_to_local(file: UploadFile):
    ...
    return RestfulResponse.success(path)
...
async def get_settings_tabs(classifys: list[str] = Body(...), auth: Auth = Depends(FullAdminAuth())):
    return RestfulResponse.success(await crud.SettingsTabDal(auth.db).get_datas(limit=0, classify=("in", classifys)))
...
async def get_settings_tabs_values(tab_id: int, auth: Auth = Depends(FullAdminAuth())):
    return RestfulResponse.success(await crud.SettingsDal(auth.db).get_tab_values(tab_id=tab_id))
...
async def put_settings_tabs_values(
    ...
    return RestfulResponse.success(await crud.SettingsDal(auth.db).update_datas(datas, request))
...
async def get_setting_base_config(db: AsyncSession = Depends(db_getter)):
    return RestfulResponse.success(await crud.SettingsDal(db).get_base_config())


@app.get("/settings/privacy", summary="获取隐私协议")
async def get_settings_privacy(auth: Auth = Depends(OpenAuth())):
    return RestfulResponse.success((await crud.SettingsDal(auth.db).get_data(config_key="web_privacy")).config_value)


@app.get("/settings/agreement", summary="获取用户协议")
async def get_settings_agreement(auth: Auth = Depends(OpenAuth())):
    return RestfulResponse.success((await crud.SettingsDal(auth.db).get_data(config_key="web_agreement")).config_value)
...
async def post_tasks(
    ...
    return RestfulResponse.success(await crud.TaskDal(db).create_task(rd, data))
...
async def put_tasks(
    ...
    return RestfulResponse.success(await crud.TaskDal(db).put_task(rd, _id, data))
...
async def delete_task(
    ...
    return RestfulResponse.success(await crud.TaskDal(db).delete_task(_id))
...
async def get_task(
    ...
    return RestfulResponse.success(await crud.TaskDal(db).get_task(_id, v_schema=schemas.TaskSimpleOut))
...
async def run_once_task(
    ...
    return RestfulResponse.success(await crud.TaskDal(db).run_once_task(rd, _id))
...
async def get_task_group_options(db: AsyncIOMotorDatabase = Depends(mongo_getter), auth: Auth = Depends(AllUserAuth())):
    return RestfulResponse.success(await crud.TaskGroupDal(db).get_datas(limit=0))
