...
async def test(auth: Auth = Depends(OpenAuth())):
    return RestfulResponse.success(await crud.TestDal(auth.db).relationship_where_operations_has())
...
async def create_user(data: schemas.UserIn, auth: Auth = Depends(FullAdminAuth(permissions=["auth.user.create"]))):
    return RestfulResponse.success(await crud.UserDal(auth.db).create_data(data=data))
...
async def put_user(
    ...
    return RestfulResponse.success(await crud.UserDal(auth.db).put_data(data_id, data))
...
async def get_user(
    ...
    options = [joinedload(model.roles), joinedload(model.depts)]
    schema = schemas.UserOut
    return RestfulResponse.success(await crud.UserDal(auth.db).get_data(data_id, v_options=options, v_schema=schema))
...
async def user_current_reset_password(data: schemas.ResetPwd, auth: Auth = Depends(AllUserAuth())):
    return RestfulResponse.success(await crud.UserDal(auth.db).reset_current_password(auth.user, data))
...
async def post_user_current_update_info(data: schemas.UserUpdateBaseInfo, auth: Auth = Depends(AllUserAuth())):
    return RestfulResponse.success(await crud.UserDal(auth.db).update_current_info(auth.user, data))
...
async def post_user_current_update_avatar(file: UploadFile, auth: Auth = Depends(AllUserAuth())):
    return RestfulResponse.success(await crud.UserDal(auth.db).update_current_avatar(auth.user, file))
...
async def post_user_export_query_list(
    ...
    return RestfulResponse.success(await crud.UserDal(auth.db).export_query_list(header, params))
...
async def get_user_download_new_import_template(auth: Auth = Depends(AllUserAuth())):
    return RestfulResponse.success(await crud.UserDal(auth.db).download_import_template())
...
async def post_import_users(file: UploadFile, auth: Auth = Depends(FullAdminAuth(permissions=["auth.user.import"]))):
    return RestfulResponse.success(await crud.UserDal(auth.db).import_users(file))
...
async def post_users_init_password(
    ...
    return RestfulResponse.success(await crud.UserDal(auth.db).init_password_send_sms(ids.ids, rd))
...
async def post_users_init_password_send_email(
    ...
    return RestfulResponse.success(await crud.UserDal(auth.db).init_password_send_email(ids.ids, rd))
...
async def create_role(role: schemas.RoleIn, auth: Auth = Depends(FullAdminAuth(permissions=["auth.role.create"]))):
    return RestfulResponse.success(await crud.RoleDal(auth.db).create_data(data=role))
...
async def put_role(
    ...
    return RestfulResponse.success(await crud.RoleDal(auth.db).put_data(data_id, data))
...
async def get_role_options(auth: Auth = Depends(FullAdminAuth(permissions=["auth.user.create", "auth.user.update"]))):
    return RestfulResponse.success(await crud.RoleDal(auth.db).get_select_datas())
...
async def get_role(
    ...
    return RestfulResponse.success(await crud.RoleDal(auth.db).get_data(data_id, v_options=options, v_schema=schema))
...
async def get_menus_treeselect(
    ...
    return RestfulResponse.success(await crud.MenuDal(auth.db).get_tree_list(mode=3))
...
async def create_menu(menu: schemas.Menu, auth: Auth = Depends(FullAdminAuth(permissions=["auth.menu.create"]))):
    ...
    return RestfulResponse.success(await crud.MenuDal(auth.db).create_data(data=menu))
...
async def get_menus(
    ...
    return RestfulResponse.success(await crud.MenuDal(auth.db).put_data(data_id, data))
...
async def put_menus(
    ...
    return RestfulResponse.success(await crud.MenuDal(auth.db).get_data(data_id, v_schema=schema))
...
async def get_dept_treeselect(auth: Auth = Depends(FullAdminAuth())):
    return RestfulResponse.success(await crud.DeptDal(auth.db).get_tree_list(mode=3))


@app.post("/depts", summary="创建部门信息")
async def create_dept(data: schemas.Dept, auth: Auth = Depends(FullAdminAuth())):
    return RestfulResponse.success(await crud.DeptDal(auth.db).create_data(data=data))
...
async def put_dept(
    ...
    return RestfulResponse.success(await crud.DeptDal(auth.db).put_data(data_id, data))
