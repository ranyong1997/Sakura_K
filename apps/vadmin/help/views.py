...
async def get_issue_categorys_options(auth: Auth = Depends(AllUserAuth())):
    ...
    return RestfulResponse.success(await crud.IssueCategoryDal(auth.db).get_datas(limit=0, is_active=True, v_schema=schema))
...
async def create_issue_category(data: schemas.IssueCategory, auth: Auth = Depends(AllUserAuth())):
    ...
    return RestfulResponse.success(await crud.IssueCategoryDal(auth.db).create_data(data=data))
...
async def put_issue_category(data_id: int, data: schemas.IssueCategory, auth: Auth = Depends(AllUserAuth())):
    return RestfulResponse.success(await crud.IssueCategoryDal(auth.db).put_data(data_id, data))
...
async def get_issue_category(data_id: int, auth: Auth = Depends(AllUserAuth())):
    ...
    return RestfulResponse.success(await crud.IssueCategoryDal(auth.db).get_data(data_id, v_schema=schema))
...
async def create_issue(data: schemas.Issue, auth: Auth = Depends(AllUserAuth())):
    data.create_user_id = auth.user.id
    return RestfulResponse.success(await crud.IssueDal(auth.db).create_data(data=data))
...
async def put_issue(data_id: int, data: schemas.Issue, auth: Auth = Depends(AllUserAuth())):
    return RestfulResponse.success(await crud.IssueDal(auth.db).put_data(data_id, data))
...
async def get_issue(data_id: int, db: AsyncSession = Depends(db_getter)):
    ...
    return RestfulResponse.success(await crud.IssueDal(db).get_data(data_id, v_schema=schema))
...
async def issue_add_view_number(data_id: int, db: AsyncSession = Depends(db_getter)):
    return RestfulResponse.success(await crud.IssueDal(db).add_view_number(data_id))
