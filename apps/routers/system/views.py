...
from apps.cruds import system_crud, auth_crud
from apps.cruds.system_crud import DictTypeDal
from apps.depends.Paging import IdList
from apps.depends.current import AllUserAuth, OpenAuth, FullAdminAuth
from apps.depends.validation.auth import Auth
from apps.routers.system.dict_detail import DictDetailParams
from apps.routers.system.dict_type import DictTypeParams
from apps.routers.system.task import TaskParams, TaskRecordParams
from apps.schemas import dict_schema, task_schema
...
router = APIRouter(prefix="/vadmin/system", tags=["系统管理"])
...
async def create_dict_types(data: dict_schema.DictType, auth: Auth = Depends(AllUserAuth())):
    return RestfulResponse.success(await DictTypeDal(auth.db).create_data(data=data))
...
async def get_dicts_options(auth: Auth = Depends(AllUserAuth())):
    return RestfulResponse.success(await system_crud.DictTypeDal(auth.db).get_select_datas())


@router.put("/dict/types/{data_id}", summary="更新字典类型")
async def put_dict_types(data_id: int, data: dict_schema.DictType, auth: Auth = Depends(AllUserAuth())):
    return RestfulResponse.success(await system_crud.DictTypeDal(auth.db).put_data(data_id, data))


@router.get("/dict/types/{data_id}", summary="获取字典类型详细")
async def get_dict_type(data_id: int, auth: Auth = Depends(AllUserAuth())):
    schema = dict_schema.DictTypeSimpleOut
    return RestfulResponse.success(await system_crud.DictTypeDal(auth.db).get_data(data_id, v_schema=schema))
...
async def create_dict_details(data: dict_schema.DictDetails, auth: Auth = Depends(AllUserAuth())):
    return RestfulResponse.success(await system_crud.DictDetailsDal(auth.db).create_data(data=data))
...
async def get_dict_details(params: DictDetailParams = Depends(), auth: Auth = Depends(AllUserAuth())):
    datas, count = await system_crud.DictDetailsDal(auth.db).get_datas(**params.dict(), v_return_count=True)
    return RestfulResponse.success(datas, count=count)
...
async def delete_dict_details(ids: IdList = Depends(), auth: Auth = Depends(AllUserAuth())):
    await system_crud.DictDetailsDal(auth.db).delete_datas(ids.ids)
    return RestfulResponse.success("删除成功")


@router.put("/dict/details/{data_id}", summary="更新字典元素")
async def put_dict_details(data_id: int, data: dict_schema.DictDetails, auth: Auth = Depends(AllUserAuth())):
    return RestfulResponse.success(await system_crud.DictDetailsDal(auth.db).put_data(data_id, data))


@router.get("/dict/details/{data_id}", summary="获取字典元素详情")
async def get_dict_detail(data_id: int, auth: Auth = Depends(AllUserAuth())):
    schema = dict_schema.DictDetailsSimpleOut
    return RestfulResponse.success(await system_crud.DictDetailsDal(auth.db).get_data(data_id, v_schema=schema))
...
async def upload_image_to_oss(file: UploadFile, path: str = Form(...)):
    result = await AliyunOSS(BucketConf(**settings.settings.oss.ALIYUN_OSS)).upload_image(path, file)
...
async def upload_video_to_oss(file: UploadFile, path: str = Form(...)):
    result = await AliyunOSS(BucketConf(**settings.settings.oss.ALIYUN_OSS)).upload_video(path, file)
    return RestfulResponse.success(result)


@router.post("/upload/file/to/oss", summary="上传文件到阿里云OSS")
async def upload_file_to_oss(file: UploadFile, path: str = Form(...)):
    result = await AliyunOSS(BucketConf(**settings.settings.oss.ALIYUN_OSS)).upload_file(path, file)
    return RestfulResponse.success(result)


@router.post("/upload/file/local", summary="上传文件到本地")
async def upload_file_to_local(file: UploadFile):
    ...
    return RestfulResponse.success(data=path)
...
async def sms_send(
        telephone: str,
        rd: Redis = Depends(AsyncAbstractDatabase.db_getter),
        auth: Auth = Depends(OpenAuth())
):
    user = await auth_crud.UserDal(auth.db).get_data(telephone=telephone, v_return_none=True)
    if not user:
        return RestfulResponse.error(message="手机号不存在！")
    sms = CodeSMS(telephone, rd)
    return RestfulResponse.success(await sms.main_async())
...
async def get_settings_tabs(classifys: list[str] = Body(...), auth: Auth = Depends(FullAdminAuth())):
    return RestfulResponse.success(
        await system_crud.SettingsTabDal(auth.db).get_datas(limit=0, classify=("in", classifys))
    )
...
async def get_settings_tabs_values(tab_id: int, auth: Auth = Depends(FullAdminAuth())):
    return RestfulResponse.success(await system_crud.SettingsDal(auth.db).get_tab_values(tab_id=tab_id))
...
async def put_settings_tabs_values(
    ...
    return RestfulResponse.success(await system_crud.SettingsDal(auth.db).update_datas(datas, request))


@router.get("/settings/base/config", summary="获取系统基础配置", description="每次进入系统中时使用")
async def get_setting_base_config(db: AsyncSession = Depends(ORMDatabase.db_getter)):
    return RestfulResponse.success(await system_crud.SettingsDal(db).get_base_config())


@router.get("/settings/privacy", summary="获取隐私协议")
async def get_settings_privacy(auth: Auth = Depends(OpenAuth())):
    return RestfulResponse.success(
        (await system_crud.SettingsDal(auth.db).get_data(config_key="web_privacy")).config_value
    )
...
async def get_settings_agreement(auth: Auth = Depends(OpenAuth())):
    return RestfulResponse.success(
        (await system_crud.SettingsDal(auth.db).get_data(config_key="web_agreement")).config_value
    )
...
async def get_tasks(
    ...
    datas, count = await system_crud.TaskDal(db).get_tasks(**p.dict())
    return RestfulResponse.success(datas, count=count)


@router.post("/tasks", summary="添加定时任务")
async def post_tasks(
        data: task_schema.Task,
        db: AsyncIOMotorDatabase = Depends(ORMDatabase.db_getter),
        rd: Redis = Depends(RedisDatabase.db_getter),
        auth: Auth = Depends(AllUserAuth())
):
    return RestfulResponse.success(await system_crud.TaskDal(db).create_task(rd, data))
...
async def put_tasks(
    ...
    return RestfulResponse.success(await system_crud.TaskDal(db).put_task(rd, _id, data))
...
async def delete_task(
    ...
    return RestfulResponse.success(await system_crud.TaskDal(db).delete_task(_id))
...
async def get_task(
    ...
    return RestfulResponse.success(await system_crud.TaskDal(db).get_task(_id, v_schema=task_schema.TaskSimpleOut))
...
async def run_once_task(
    ...
    return RestfulResponse.success(await system_crud.TaskDal(db).run_once_task(rd, _id))
...
async def get_task_group_options(
    ...
    return RestfulResponse.success(await system_crud.TaskGroupDal(db).get_datas(limit=0))
...
async def get_task_records(
    ...
    count = await system_crud.TaskRecordDal(db).get_count(**p.to_count())
    datas = await system_crud.TaskRecordDal(db).get_datas(**p.dict())
...
#                    探针检测网页存活                        #
###########################################################
@router.get("/status", summary="探针检测网页存活")
async def get_status(
    ...
    try:
        ...
        if web_urls_after_base64:
            web_urls = system_crud.deocde_web_urls(web_urls_after_base64)
            status["web_access"] = web_access.test_all(web_urls)
        # 返回
        return status
    except Exception as e:
        raise RestfulResponse.error(code=500, message=str(e))
